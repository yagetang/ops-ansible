...
function func_create_tomcatdir()
{
  if [[ ! -d ${appRoot} ]];then
       ...
       wget -O /tmp/apache-tomcat-8.0.50.tar.gz http://archive.apache.org/dist/tomcat/tomcat-8/v8.0.50/bin/apache-tomcat-8.0.50.tar.gz
       tar  -zxvf /tmp/apache-tomcat-8.0.50.tar.gz -C /tmp/
       mv /tmp/apache-tomcat-8.0.50 ${appRoot}
  fi
}
